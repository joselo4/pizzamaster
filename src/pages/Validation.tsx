...
import { fetchPedidoConfigMap as fetchConfigMap } from '../lib/pedidoConfig';
...
export default function Validation() {
  ...
  const [configDeliveryCost, setConfigDeliveryCost] = useState<number>(0);
  ...
  const loadDeliveryConfig = async () => {
    try {
      ...
      const raw = cfg?.costo_delivery ?? cfg?.delivery_fee ?? 0;
      const parsed = Number(raw);
      const fee = Number.isFinite(parsed) ? Math.max(0, parsed) : 0;
      setConfigDeliveryCost(fee);
      return fee;
    } catch {
      setConfigDeliveryCost(0);
      return 0;
    }
  };
  ...
  const load = async () => {
    setError('');
    const cfgDeliveryCost = await loadDeliveryConfig();
    if (!selected) setDeliveryCost(cfgDeliveryCost);
    try {
  ...
  const pick = async (r: OrderRequest) => {
    setSelected(r);
    setItems(r.items || []);
    setDeliveryCost(r.service_type==='Delivery' ? configDeliveryCost : 0);
    setNotes(r.notes || '');
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-4 md:py-6">
      <header className="mb-4 flex flex-wrap items-center gap-3">
        <h1 className="text-xl font-black md:text-2xl text-white">Validación</h1>
        <div className="ml-auto flex items-center gap-2 shrink-0">
          <div className="flex w-48 sm:w-64 items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-white shadow-sm focus-within:border-orange-500/80 transition-all">
            <Search size={16} className="text-white/40" />
            <input className="w-full bg-transparent outline-none text-sm placeholder:text-white/30 text-white" placeholder="Buscar (opcional)" />
          </div>
          
          <button type="button" className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 px-4 py-2 text-sm font-black text-white shadow-sm transition-all" onClick={load}>
            <RefreshCw size={16} className="text-orange-400" /> Actualizar
          </button>
        </div>
      </header>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4 shadow-sm">
            ...
            {selected && (
              <div className="space-y-4">
                ...
                <div className="rounded-xl border border-white/10 p-3">
                  ...
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                    <label className="block">
                      ...
                      <div className="flex bg-black/40 border border-white/10 rounded-lg p-0.5 h-[38px]">
                        {['Delivery', 'Local'].map((srv) => (
                          <button
                            key={srv}
                            type="button"
                            onClick={() => setServiceType(srv as any)}
                            className={`flex-1 text-center text-xs font-black rounded-md transition-all ${
                              serviceType === srv
                ...
                {/* MOTIVO DE RECHAZO PREMIUM Y LEGIBLE */}
                <div className="rounded-2xl border border-rose-500/20 bg-rose-500/5 p-4 space-y-2 animate-in fade-in duration-200">
                  ...
                  <textarea
                    className="w-full rounded-xl border border-rose-500/25 bg-black/40 p-3 text-rose-100 text-sm placeholder:text-rose-200/30 outline-none focus:border-rose-500 transition-all duration-200"
                    rows={2}
                    value={rejectReason}
                    onChange={(e) => setRejectReason(e.target.value)}
                    placeholder="Escribe el motivo de rechazo que verá el cliente..."
                  />
                  <div className="text-[10px] text-rose-350/60 leading-snug">Este motivo se le notificará al cliente en la pantalla pública de seguimiento /track.</div>
                </div>
