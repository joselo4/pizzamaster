...
export default function Kitchen() {
  ...
  useEffect(() => {
    ...
    // Live ticking every 30 seconds to update wait times
      ...
      >
        {orders.length === 0 ? (
            ...
            orders.map(order => {
              ...
              const isDelayed = elapsedMins >= 10 && order.status === 'Pendiente';
              ...
              return (
                <div 
                  ...
                  className={`
                    ...
                    snap-center flex flex-col shadow-2xl transition-all duration-300 overflow-hidden
                    ${isDelayed 
                      ? 'border-red-500 shadow-[0_0_30px_rgba(239,68,68,0.25)] animate-pulse-glow hover:scale-[1.02]' 
                      : order.status === 'Pendiente' 
                        ? 'border-orange-500 shadow-orange-950/20 hover:scale-[1.01]' 
                        : 'border-yellow-500 shadow-yellow-950/20 hover:scale-[1.01]'
                    }
                  `}
                >
                  <div className={`p-4 ${isDelayed ? 'bg-gradient-to-r from-red-650 to-red-500 animate-pulse' : order.status === 'Pendiente' ? 'bg-orange-600' : 'bg-yellow-600'} text-white font-bold flex flex-col gap-1 rounded-t-xl shrink-0`}>
                      <div className="flex justify-between items-center">
                          <span className="text-lg flex items-center gap-1.5 font-black">
                            #{order.id}
                            {isDelayed && (
                              <span className="text-[9px] bg-white text-red-600 px-2 py-0.5 rounded-full font-black animate-bounce tracking-wide shrink-0">
                                🚨 DEMORA!
                              </span>
                            )}
                          </span>
                          <span className="text-xs bg-black/30 px-2.5 py-1 rounded-xl flex items-center gap-1 font-extrabold">
                            <Clock size={12}/> 
                            {new Date(order.created_at).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})}
                            <span className="text-white/60">({elapsedMins}m)</span>
                          </span>
                      </div>
