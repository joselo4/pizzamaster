...
function Card({ title, value, hint }: { title: string; value: string | number; hint?: string }) {
  return (
    <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-card p-4 shadow-xs">
      <div className="text-xs uppercase tracking-wide text-zinc-500 dark:text-white/60">{title}</div>
      <div className="mt-1 text-2xl font-black text-zinc-900 dark:text-white">{value}</div>
      {hint && <div className="mt-1 text-xs text-zinc-400 dark:text-white/50">{hint}</div>}
    </div>
  );
}
  ...
  return (
    <div className="p-4 text-zinc-900 dark:text-white">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <div className="text-xl font-black text-zinc-900 dark:text-white">Métricas</div>
          <div className="text-sm text-zinc-500 dark:text-white/50">Filtro: {range.label}</div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <select
            className="rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 px-3 py-2 text-sm text-zinc-900 dark:text-white outline-none focus:border-orange-500/50"
            value={rangeMode}
          ...
          <button
            onClick={() => void load()}
            className="rounded-xl bg-orange-600 px-4 py-2 text-sm font-black text-white hover:bg-orange-500 transition shadow-md"
          >
            Actualizar
          </button>
        </div>
      ...
      {err && (
        <div className="mt-4 rounded-xl border border-red-200 dark:border-red-800 bg-red-100 dark:bg-red-900/20 p-3 text-sm text-red-700 dark:text-red-200">
      ...
      {loading ? (
        <div className="mt-6 text-zinc-550 dark:text-white/60">Cargando…</div>
      ) : (
        <>
          ...
          <div className="mt-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-card p-4 shadow-xs">
            <div className="font-black text-zinc-850 dark:text-white mb-3">Método de pago (conteo)</div>
