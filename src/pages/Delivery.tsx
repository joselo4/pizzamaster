            ...
            visibleOrders.map(order => (
              <div key={order.id} className={`relative flex-shrink-0 w-[85vw] max-w-sm h-[95%] bg-card rounded-2xl border-2 snap-center flex flex-col shadow-2xl overflow-hidden transition-all ${selectedIds.includes(order.id) ? 'border-blue-500 shadow-blue-500/20' : 'border-blue-900/50'}`}>
                 ...
                 <div className={`p-4 ${order.status === 'Listo' ? 'bg-blue-900/40' : 'bg-green-900/40'} border-b border-gray-700 flex justify-between items-center shrink-0`}>
                    ...
                    <div>
                        <h3 className="text-xl font-black text-white mb-2">{order.client_name}</h3>
                        {order.service_type === 'Delivery' && (
                          <div className="bg-blue-950/30 border border-blue-500/30 rounded-2xl p-3.5 space-y-3 shadow-md">
                            <div className="flex items-start gap-2 text-blue-200 text-sm">
                              <MapPin size={16} className="mt-0.5 shrink-0 text-blue-400" />
                              <span className="font-extrabold leading-tight">{order.client_address || 'Sin dirección'}</span>
                            </div>
                            <div className="flex gap-2">
                              <button onClick={(e) => {e.stopPropagation(); openMap(order.client_address || '')}} className="flex-1 bg-blue-600/20 hover:bg-blue-600/35 border border-blue-500/40 text-blue-300 py-2 rounded-xl text-xs font-black flex items-center justify-center gap-1.5 transition-all"><Navigation size={14}/> MAPA</button>
                              <button onClick={(e) => {e.stopPropagation(); callClient(order.client_phone)}} className="flex-1 bg-emerald-600/20 hover:bg-emerald-600/35 border border-emerald-500/40 text-emerald-300 py-2 rounded-xl text-xs font-black flex items-center justify-center gap-1.5 transition-all"><Phone size={14}/> LLAMAR</button>
                            </div>
                          </div>
                        )}
                    </div>
