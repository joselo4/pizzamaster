-- PATCH: bloqueo exponencial de login (máximo 5 intentos incorrectos)
-- Ejecutar en Supabase SQL Editor si la base de datos ya existe.
-- Reemplaza public.rpc_login para devolver locked_until y aumentar el bloqueo progresivamente.
...
begin
  ...
  if u.pin_hash is null or u.pin_hash <> extensions.crypt(trim(p_pin), u.pin_hash) then
    ...
    if fails >= max_fails then
      -- Bloqueo exponencial: intento 5 = 5 min, 6 = 10 min, 7 = 20 min, 8 = 40 min,
      -- con tope de 240 min. El contador se reinicia al quedar fuera de la ventana.
      ...
      return jsonb_build_object(
        ...
        'message', 'Acceso bloqueado por demasiados intentos'
      );
    end if;
