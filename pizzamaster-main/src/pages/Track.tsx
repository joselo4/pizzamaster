...
{data?.status === 'Rechazado' && (data as any)?.reject_reason?.trim() && (
  <div className="mt-4 rounded border border-red-300 bg-red-50 p-3 text-sm text-red-800">
    <div className="font-semibold">Pedido rechazado
</div>
    <div className="mt-1 whitespace-pre-wrap">{(data as any).reject_reason}</div>
  </div>
)}
</div>
  );
