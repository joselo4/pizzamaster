      ...
      <div className="bg-card border border-white/10 rounded-2xl p-4">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            <label className="block text-sm text-white/70 mb-1">Chat ID</label>
            <input type="text" value={telegramValidationChatId} onChange={(e) => setTelegramValidationChatId(e.target.value)} className="w-full rounded-lg bg-white text-gray-900 px-3 py-2" placeholder="-1001234567890" />
          </div>
          <div className="rounded-xl border border-white/10 bg-white/5 p-4">
            <label className="block text-sm text-white/70 mb-1">Estados que disparan la alerta</label>
            <input type="text" value={telegramValidationStatuses} onChange={(e) => setTelegramValidationStatuses(e.target.value)} className="w-full rounded-lg bg-white text-gray-900 px-3 py-2" placeholder="Nuevo,En Revisión,Validación" />
          </div>
        </div>

        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
          <label className="block text-sm text-white/70 mb-1">Plantilla del mensaje</label>
          <textarea value={telegramValidationTemplate} onChange={(e) => setTelegramValidationTemplate(e.target.value)} rows={8} className="w-full rounded-lg bg-white text-gray-900 px-3 py-2" />
          <div className="text-xs text-white/50 mt-2">Placeholders: <code>{'{{order_id}}'}</code>, <code>{'{{request_id}}'}</code>, <code>{'{{status}}'}</code>, <code>{'{{client_name}}'}</code>, <code>{'{{client_phone}}'}</code>, <code>{'{{client_address}}'}</code>, <code>{'{{service_type}}'}</code>, <code>{'{{total}}'}</code>, <code>{'{{notes}}'}</code>, <code>{'{{created_at}}'}</code></div>
        </div>
