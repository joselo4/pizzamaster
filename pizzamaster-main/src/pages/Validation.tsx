...
const onReject = async (orderId) => {
  if (!rejectionReason.trim()) { alert('Debes indicar un motivo'); return; }
  try {
    const { error } = await supabase
      .from('orders')
      .update({ status: 'RECHAZADO', rejection_reason: rejectionReason.trim() })
      .eq('id', orderId);
...
{decision === 'RECHAZADO' && (
  <div className="mt-4">
    ...
    <textarea className="mt-1 w-full rounded border p-2" rows={3} value={rejectionReason} onChange={(e)=>setRejectionReason(e.target.value)} placeholder="Describe el motivo (obligatorio)"/>
