      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        {tab === 'usuarios' && (
            <div>
                ...
                {isModal && (<div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50"><form onSubmit={saveUser} className="bg-card p-6 rounded-xl border border-gray-700 w-full max-w-sm max-h-[90vh] overflow-y-auto"><h3 className="text-xl font-bold mb-4">{editItem.id ? 'Editar' : 'Nuevo'} Usuario</h3><div className="space-y-4"><div><label htmlFor="user_username" className="text-xs text-gray-400 block mb-1">Nombre</label><input id="user_username" name="username" autoComplete="username" className="w-full bg-dark p-3 rounded border border-gray-600" value={editItem.username || ''} onChange={e => setEditItem({...editItem, username: e.target.value})} /></div><div><label htmlFor="user_pin" className="text-xs text-gray-400 block mb-1">PIN</label><input id="user_pin" name="pin" autoComplete="one-time-code" className="w-full bg-dark p-3 rounded border border-gray-600" type="tel" maxLength={4} value={editItem.pin || ''} onChange={e => setEditItem({...editItem, pin: e.target.value})} /></div><div className="bg-dark/50 p-3 rounded border border-gray-700"><label className="text-xs font-bold text-orange-500 block mb-2 flex items-center gap-1"><Shield size={12}/> PERMISOS DE ACCESO</label><div className="space-y-2">{availablePermissions.map(p => (<label key={p.id} className="flex items-center gap-3 cursor-pointer p-2 hover:bg-white/5 rounded transition-colors"><div className={`w-5 h-5 rounded border flex items-center justify-center ${selectedPerms.includes(p.id) ? 'bg-green-600 border-green-500' : 'border-gray-500'}`}>{selectedPerms.includes(p.id) && <CheckSquare size={14} className="text-white"/>}</div><input id={`perm_${p.id}`} name={`perm_${p.id}`} type="checkbox" className="hidden" checked={selectedPerms.includes(p.id)} onChange={() => togglePermission(p.id)} /><span className="text-sm text-gray-300">{p.label}</span></label>))}</div></div></div><div className="flex gap-2 mt-6"><button type="button" onClick={() => setIsModal(false)} className="flex-1 bg-gray-700 p-3 rounded text-sm font-bold">Cancelar</button><button className="flex-1 bg-orange-600 p-3 rounded text-sm font-bold shadow-lg">Guardar</button></div></form></div>)}
            </div>
        )}
        ...
        {tab === 'clientes' && (
             <div>
                 ...
                 <div className="grid gap-2 whitespace-normal break-words max-w-full leading-snug">{filteredData.map((i: any, idx: number) => (<div key={`${String(i.phone || '')}-${idx}`} className="bg-card p-3 rounded border border-gray-800 flex justify-between items-center"><div><div className="font-bold flex items-center gap-2"><Users size={16} className="text-blue-500"/> {i.name}</div><div className="text-xs text-gray-400">{i.phone} {i.address && `(${i.address})`}</div></div><div className="flex gap-3"><button onClick={() => openClientModal(i)} className="text-blue-400"><Edit size={18}/></button><button onClick={() => del('customers', i.phone)} className="text-red-500"><Trash2 size={18}/></button></div></div>))}</div>
                 {isClientModal && (<div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50"><form onSubmit={saveClient} className="bg-card p-6 rounded-xl border border-gray-700 w-full max-w-sm"><h3 className="text-xl font-bold mb-4">{editItem.phone ? 'Editar' : 'Nuevo'} Cliente</h3><label htmlFor="client_phone" className="text-xs text-gray-400 block mb-1">Teléfono</label><input id="client_phone" name="client_phone" autoComplete="tel" className="w-full bg-dark p-3 rounded mb-3 border border-gray-600" placeholder="Teléfono" value={editItem.phone || ''} onChange={e => setEditItem({...editItem, phone: e.target.value})} disabled={!!editItem.phone && !!data.find(c => c.phone === editItem.phone)} /><label htmlFor="client_name" className="text-xs text-gray-400 block mb-1">Nombre</label><input id="client_name" name="client_name" autoComplete="name" className="w-full bg-dark p-3 rounded mb-3 border border-gray-600" placeholder="Nombre" value={editItem.name || ''} onChange={e => setEditItem({...editItem, name: e.target.value})} /><label htmlFor="client_address" className="text-xs text-gray-400 block mb-1">Dirección</label><input id="client_address" name="client_address" autoComplete="street-address" className="w-full bg-dark p-3 rounded mb-4 border border-gray-600" placeholder="Dirección" value={editItem.address || ''} onChange={e => setEditItem({...editItem, address: e.target.value})} /><div className="flex gap-2"><button type="button" onClick={() => setIsClientModal(false)} className="flex-1 bg-gray-700 p-3 rounded">Cancelar</button><button className="flex-1 bg-blue-600 p-3 rounded font-bold">Guardar</button></div></form></div>)}
             </div>
        )}
...
{tab === 'productos' && (
  <div className="space-y-4">
    <div className="rounded-2xl border border-orange-500/20 bg-gradient-to-br from-orange-500/10 via-card to-card p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h2 className="text-2xl font-black">Productos y tags de /pedidos</h2>
          <p className="mt-1 text-sm text-gray-300">Crea tags nuevos, asigna productos existentes o nuevos, y oculta productos sin borrarlos.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={() => { setSelectedPerms([]); setEditItem({ active: true, category: productCategories.find(c => c !== 'Promo') || 'Pizzas' }); setIsModal(true); }} className="bg-green-600 px-3 py-2 rounded-xl font-bold flex gap-2 items-center shadow-lg">
            <Plus size={18}/> Nuevo producto
          </button>
          <button onClick={saveProductOrder} disabled={savingProdOrder || !Array.isArray(data) || data.length === 0}
            className="bg-orange-600 px-3 py-2 rounded-xl font-bold flex gap-2 items-center disabled:opacity-60">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_auto]">
        <div>
          <div className="mb-2 text-xs font-bold uppercase tracking-wider text-orange-200">Tags visibles en /pedidos</div>
          <div className="flex flex-wrap gap-2">
            {pedidoCategories.map((cat, idx) => (
              <span
                ...
                className="inline-flex cursor-move items-center gap-1 rounded-full border border-white/10 bg-white/10 px-2 py-2 text-sm font-bold"
                title="Arrastra para ordenar o usa las flechas. Se auto-guarda al cambiar."
              >
                <GripVertical size={14} className="text-gray-300" />
                <button type="button" disabled={idx === 0} onClick={() => movePedidoCategory(cat, -1)} className="rounded-full bg-black/25 px-2 text-white/80 disabled:opacity-30" title="Mover a la izquierda">←</button>
                <span className="px-1">{cat}</span>
                <button type="button" disabled={idx === pedidoCategories.length - 1} onClick={() => movePedidoCategory(cat, 1)} className="rounded-full bg-black/25 px-2 text-white/80 disabled:opacity-30" title="Mover a la derecha">→</button>
                <button type="button" onClick={() => removePedidoCategory(cat)} className="rounded-full bg-black/25 p-1 text-red-200 hover:bg-red-500/20" title="Ocultar tag"><X size={14}/></button>
              </span>
            ))}
          </div>
        </div>
        <div className="flex gap-2">
          <input list="pedido-category-options" className="bg-dark px-3 py-2 rounded-xl border border-gray-700 w-full sm:w-64" placeholder="Nuevo tag: Hamburguesas" value={newPedidoCategory} onChange={e => setNewPedidoCategory(e.target.value)} />
          <button type="button" onClick={addPedidoCategory} className="bg-blue-600 px-4 py-2 rounded-xl font-black">Agregar tag</button>
        </div>
      </div>
    ...
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div className="text-xs text-gray-400">
        Tip: ordena los tags arrastrando o usando las flechas ← →. El cambio se auto-guarda al soltar/mover; también puedes presionar <b>Guardar orden</b>. Para quitar de la tienda usa <b>Ocultar</b>; para borrar definitivo usa el basurero.
      </div>
      <div className="flex items-center gap-2">
        <input className="bg-dark px-3 py-2 rounded-xl border border-gray-700 w-full sm:w-72" placeholder="Buscar producto..."
          value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        <button onClick={() => setSearchTerm('')} className="bg-gray-800 px-3 py-2 rounded-xl border border-gray-700" title="Limpiar">
          <X size={18}/>
        </button>
      </div>
    </div>
    ...
    <div className="grid gap-2">
      {(productsView as any[]).map((p: any, idx: number) => {
        ...
        <div key={p.id} draggable onDragStart={() => setDragProdId(String(p.id))} onDragOver={(e) => e.preventDefault()}
          ...
          className={`bg-card p-3 rounded-2xl border flex justify-between items-center gap-3 ${visible ? 'border-gray-800' : 'border-red-500/30 opacity-70'} ${dragProdId === String(p.id) ? 'ring-2 ring-orange-500/60' : ''}`}>
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-sm text-orange-500 font-bold">S/ {Number(p.price || 0).toFixed(2)}</div>
              <div className="text-[11px] text-gray-400">Orden: <b>{p.sort_index ?? (idx + 1)}</b> • Tag: {p.category || '—'}{p.is_promo ? ' • Promo' : ''} • {visible ? 'Visible' : 'Oculto'}</div>
            </div>
          </div>
          <div className="flex gap-3 shrink-0 items-center">
            <button onClick={() => toggleProductActive(p)} className={visible ? 'text-yellow-300' : 'text-emerald-400'} title={visible ? 'Ocultar en /pedidos' : 'Mostrar en /pedidos'}>
              {visible ? <Eye size={18}/> : <CheckSquare size={18}/>} 
            </button>
            <button onClick={() => { setSelectedPerms([]); setEditItem({ ...p, active: p.active !== false }); setIsModal(true); }} className="text-blue-400" title="Editar"><Edit size={18}/></button>
            <button onClick={() => del('products', p.id)} className="text-red-500" title="Eliminar definitivamente"><Trash2 size={18}/></button>
          </div>
        </div>
      );})}
    </div>
    ...
    {isModal && !selectedPerms.length && (
      <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
        <form onSubmit={saveProd} className="bg-card p-6 rounded-2xl border border-gray-700 w-full max-w-md">
          <h3 className="text-xl font-bold mb-4">{editItem.id ? 'Editar' : 'Nuevo'} producto</h3>
          <label className="text-xs text-gray-400 block mb-1">Nombre</label>
          <input className="w-full bg-dark p-3 rounded-xl mb-3 border border-gray-600" placeholder="Ej: Pizza americana familiar" value={editItem.name || ''} onChange={e => setEditItem({ ...editItem, name: e.target.value })} />
          <label className="text-xs text-gray-400 block mb-1">Precio</label>
          <input type="number" step="0.01" className="w-full bg-dark p-3 rounded-xl mb-3 border border-gray-600" placeholder="Precio" value={editItem.price || ''} onChange={e => setEditItem({ ...editItem, price: e.target.value })} />
          <label className="text-xs text-gray-400 block mb-1">Orden</label>
          <input type="number" className="w-full bg-dark p-3 rounded-xl mb-3 border border-gray-600" placeholder="Orden (opcional)" value={editItem.sort_index || ''} onChange={e => setEditItem({ ...editItem, sort_index: e.target.value })} />
          <label className="text-xs text-gray-400 block mb-1">Tag/Categoría donde aparecerá</label>
          <input list="pedido-category-options" className="w-full bg-dark p-3 rounded-xl mb-3 border border-gray-600" placeholder="Pizzas, Bebidas, Extras u otro" value={editItem.category || ''} onChange={e => setEditItem({ ...editItem, category: e.target.value })} />
          <label className="flex items-center gap-2 text-sm mb-3 select-none rounded-xl border border-white/10 bg-white/5 p-3">
            <input type="checkbox" checked={editItem.active !== false} onChange={(e) => setEditItem({ ...editItem, active: e.target.checked })} />
            <span>Visible en /pedidos</span>
          </label>
          <label className="flex items-center gap-2 text-sm mb-4 select-none rounded-xl border border-white/10 bg-white/5 p-3">
            <input type="checkbox" checked={!!editItem.is_promo} onChange={(e) => setEditItem({ ...editItem, is_promo: e.target.checked })} />
            <span>Mostrar también en el tag <b>Promo</b></span>
          </label>
          <button className="w-full bg-orange-600 py-3 rounded-xl font-bold flex justify-center gap-2 items-center"><Save size={18}/> Guardar</button>
          <button type="button" onClick={() => setIsModal(false)} className="mt-3 w-full bg-gray-700 py-3 rounded-xl font-bold">Cancelar</button>
        </form>
      </div>
    )}
  </div>
               ...
               <div className="lg:col-span-2 space-y-6">
                   ...
                   <div className="bg-card p-5 rounded-xl border border-gray-800 shadow-lg">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                           <div><label className="text-xs text-gray-400 font-bold uppercase" htmlFor="facebook">Facebook</label><div className="flex items-center gap-2 bg-dark p-3 rounded border border-gray-700"><Facebook size={14} className="text-blue-500"/><input className="bg-transparent w-full outline-none" value={config.facebook || ''} onChange={e => setConfig({...config, facebook: e.target.value})}/></div></div>
                           <div><label className="text-xs text-gray-400 font-bold uppercase" htmlFor="instagram">Instagram</label><div className="flex items-center gap-2 bg-dark p-3 rounded border border-gray-700"><Instagram size={14} className="text-purple-500"/><input className="bg-transparent w-full outline-none" value={config.instagram || ''} onChange={e => setConfig({...config, instagram: e.target.value})}/></div></div>
                           <div><label className="text-xs text-gray-400 font-bold uppercase" htmlFor="tiktok">TikTok</label><div className="flex items-center gap-2 bg-dark p-3 rounded border border-gray-700"><Video size={14} className="text-pink-500"/><input className="bg-transparent w-full outline-none" value={config.tiktok || ''} onChange={e => setConfig({...config, tiktok: e.target.value})}/></div></div>
