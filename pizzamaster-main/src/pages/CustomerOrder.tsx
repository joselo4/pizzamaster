...
import {ShoppingCart, MapPin, Phone, Clock, Plus, Minus, Trash2, Pizza, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[#0b0b0d] text-white">
      ...
      <main className="mx-auto max-w-6xl px-4 py-4 pb-28">
        ...
        <Link to="/promo?ref=pedido_top" className="mb-3 flex items-center justify-between gap-3 rounded-[24px] border border-orange-300/25 bg-[linear-gradient(135deg,rgba(251,146,60,0.18),rgba(24,24,27,0.92))] px-4 py-3 shadow-[0_18px_45px_-34px_rgba(251,146,60,0.85)] transition hover:border-orange-300/55 hover:bg-orange-500/10">
          <span className="flex min-w-0 items-center gap-3">
            ...
            <span className="min-w-0">
              <span className="block text-sm font-black text-white sm:text-base">🔥 Ver promociones</span>
              <span className="block truncate text-xs font-semibold text-orange-100/70 sm:text-sm">Ofertas disponibles antes de confirmar.</span>
            </span>
          </span>
          <span className="shrink-0 rounded-2xl bg-white px-4 py-2 text-xs font-black text-zinc-950 shadow-[0_12px_24px_-18px_rgba(255,255,255,0.7)]">Entrar</span>
        </Link>

        <section className="mb-4 rounded-[28px] border border-white/12 bg-[radial-gradient(circle_at_top_left,rgba(251,146,60,0.14),transparent_30%),linear-gradient(180deg,rgba(255,255,255,0.085),rgba(255,255,255,0.035))] p-4 shadow-[0_24px_60px_-38px_rgba(15,23,42,0.98)] backdrop-blur-sm sm:p-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl">
              <h1 className="text-2xl font-black tracking-tight text-white sm:text-4xl">{isPedidosPage ? 'Antojo resuelto: elige, paga fácil y recibe caliente' : 'Pide rápido y confirma en minutos'}</h1>
              <p className="mt-2 text-sm leading-6 text-white/70 sm:text-[15px]">{isPedidosPage ? 'Promos, pizzas, bebidas y extras en una vitrina rápida: toca una categoría, mira el precio y manda tu pedido en minutos.' : 'Elige tus productos, agrega al carrito y completa tu pedido sin pasos innecesarios.'}</p>
              <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-3">
                <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-white/75"><span className="block font-black text-white">1. Elige</span><span className="text-white/55">producto o promo</span></div>
                <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-white/75"><span className="block font-black text-white">2. Agrega</span><span className="text-white/55">al carrito</span></div>
                <div className="rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-white/75"><span className="block font-black text-white">3. Confirma</span><span className="text-white/55">y rastrea</span></div>
              </div>
            </div>
        ...
        <section className="grid grid-cols-1 gap-4 lg:grid-cols-12">
          <div className="lg:col-span-8 xl:col-span-9">
            <div className="mb-3 flex gap-2 overflow-x-auto pb-2">{categories.map((c) => {
              ...
              return (
              <button key={c} type="button" onClick={() => { hasUserChosenCategoryRef.current = true; setCategory(c); }} className={`shrink-0 rounded-full border px-4 py-2.5 text-sm font-black transition-all duration-200 ${category === c ? 'scale-[1.02] border-orange-300/50 bg-gradient-to-r from-orange-500 to-amber-400 text-slate-950 shadow-[0_16px_34px_-18px_rgba(251,146,60,0.95)]' : 'border-white/10 bg-white/5 text-white/75 hover:border-orange-200/30 hover:bg-white/10 hover:text-white'}`}>
                <span>{c}</span><span className="ml-2 rounded-full bg-black/20 px-2 py-0.5 text-[11px]">{count}</span>
              </button>
            ); })}</div>
            ...
            {filtered.length === 0 && (
              <div className="rounded-[28px] border border-dashed border-white/15 bg-white/[0.04] p-8 text-center">
                <div className="text-2xl font-black text-white">Pronto agregaremos productos aquí</div>
                <p className="mt-2 text-sm text-white/60">Esta categoría se puede llenar desde Admin → Productos, sin tocar código.</p>
              </div>
            )}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-white/10 bg-[#0b0b0d]/95 backdrop-blur" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
