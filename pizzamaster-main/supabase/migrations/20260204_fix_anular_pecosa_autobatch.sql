...
-- 20260204_fix_anular_pecosa_autobatch.sql
-- Fix QUIRÚRGICO: si una PECOSA histórica tiene movimientos OUT sin batch_id,
-- al ANULAR se crea un lote automático y se usa para la reversa IN.
-- Mantiene la regla: IN normal sigue con lote; esto solo afecta ANULACION.

create extension if not exists pgcrypto;

-- Asegurar reversal_of existe
...
begin
  ...
  for m in select * from public.movements where type='OUT' and pecosa_ref = p_pecosa_ref order by created_at asc loop
    ...
    -- Si no hay lote, crear uno automático para poder revertir stock y cumplir validaciones
    if v_batch_id is null then
      ...
      insert into public.batches(
        ...
        m.program_id, m.product_id, v_batch_code, null,
    ...
    -- Ajustar stock por lote (siempre, porque ahora hay lote)
